                ...
                : (balance?.details ?? []).map((d, i) => {
                    ...
                    return (
                      <div key={i} style={s.balanceRow}>
                        ...
                        <button style={s.settleBtn} onClick={() => toast.info(isOwed ? `Reminder sent to ${other?.name ?? 'flatmate'}` : 'Open Payments to pay')}>
                          {isOwed ? 'Remind' : 'Pay'}
                        </button>
                      </div>
                    );
...
const s: Record<string, React.CSSProperties> = {
  ...
  tabRow:     { display: 'flex', borderBottom: '0.5px solid var(--border-light)', marginBottom: '0.5rem', overflowX: 'auto' },
  tab:        { padding: '0.4rem 0.8rem', fontSize: 12.5, cursor: 'pointer', color: 'var(--text-secondary)', background: 'transparent', border: 'none', borderBottom: '2px solid transparent', marginBottom: -1, fontFamily: 'var(--font-sans)', whiteSpace: 'nowrap' },
